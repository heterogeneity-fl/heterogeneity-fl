...
class MultiClassHingeLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return loss
# don't really understand the method below
    ...
    def _compute_xi(self, s, aug, y):
        ...
        _, y_star = torch.max(aug, 1)# max of all elements in aug
        # xi_max: one-hot encoding of maximal indices
        xi_max = torch.eq(y_star[:, None], self._range).float()# element wise equality 
